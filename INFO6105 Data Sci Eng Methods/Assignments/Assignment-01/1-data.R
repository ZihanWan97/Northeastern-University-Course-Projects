...
## filter rows of Grad.Rate>=80: df4 <- filter(df,Grad.Rate>=80)
## select columns of X and Grad.Rate: df5 <- select(df4,X,Grad.Rate)
## arrange rows in descending order of Grad.Rate: df6 <- arrange(df5,desc(Grad.Rate))
## put codes above together:
df6 <- arrange(select(filter(df,Grad.Rate>=80),X,Grad.Rate),desc(Grad.Rate))
## view result
df6
...
## group by Private in dataset 'df'
df7 <- group_by(df,Private)
## summarise max and min tuition ("Outstate")
df7 <- summarise(df7,max=max(Outstate),min=min(Outstate))
## view result
df7

## group by Private in dataset 'college'
df8 <- group_by(college,Private)
## summarise max and min tuition ("Outstate")
df8 <- summarise(df8,max=max(Outstate),min=min(Outstate))
## view result
df8
